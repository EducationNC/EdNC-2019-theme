...
// EdTalk label
if ($post_type == 'edtalk') {
  echo '<span class="label-2019">Podcast</span>';
}
...
if ($category && $cat_under_column!="Reach NC Polls") {
  foreach ($category as $key=>$value) {
    if (!in_array($key, $cats_hide)) {
      ...
      echo '<span class="label-2019"><a href="' . $link . '">' . $value['cat_name'] . '</a></span> ';
    }
  }
}
// Category label
if ($category && $cat_under_column=="Reach NC Polls") {
  foreach ($category as $key=>$value) {
    if (!in_array($key, $cats_hide)) {
      $link = get_category_link($value['term_id']);
      echo '<span class="label-2019"><a href="' . $link . '" class="disabled">' . $value['cat_name'] . '</a></span> ';
    }
  }
}
...
if ($map_category) {
  foreach ($map_category as $key=>$value) {
    if (!in_array($key, $mcats_hide)) {
      ...
      echo '<span class="label-2019"><a href="' . $link . '">' . $value['name'] . '</a></span> ';
    }
  }
}
...
if ($post_type == 'flash-cards') {
  echo '<span class="label-2019"><a href="/flash-cards">Flash Cards</a></span>';
}

// Map label
if ($post_type == 'map') {
  echo '<span class="label-2019"><a href="/maps">Maps</a></span>';
}

// EdNews label
if ($post_type == 'ednews') {
  echo '<span class="label-2019">EdNews</span>';
}

// Video label
if (has_post_format('video')) {
  echo '<span class="label-2019"><span class="icon-video"></span></span>';
}
