<div class="callout-darkblue accordion" id="accordion-years" role="tablist" aria-multiselectable="true">
  <?php
  /*
  ...
  $days = $wpdb->get_results("SELECT DISTINCT DAY (post_date) AS day, MONTH( post_date ) AS month ,	YEAR( post_date ) AS year, COUNT( id ) as post_count FROM $wpdb->posts WHERE post_status = 'publish' and post_date <= now( ) and post_type = 'needtoknow' GROUP BY day, month , year ORDER BY post_date DESC");
  ...
  foreach($days as $day) :
    ...
    <?php if ($month_current != $month_prev) { ?>
      <div class="panel archive-month-ednews">
    ...
    <li class="archive-day-ednews">
      <?php
      $needtoknow = new WP_Query([
        'post_type' => 'needtoknow',
        'date_query' => [
          'year' => $day->year,
          'month' => $day->month,
          'day' => $day->day
      ...
      if ($needtoknow->have_posts()) : while ($needtoknow->have_posts()) : $needtoknow->the_post(); ?>
