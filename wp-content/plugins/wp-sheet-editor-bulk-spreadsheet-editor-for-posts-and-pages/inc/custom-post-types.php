...
if ( !class_exists( 'WPSE_Custom_Post_Types_Teaser' ) ) {
    class WPSE_Custom_Post_Types_Teaser
    {
        ...
        function init()
        {
            ...
            // We will offer this from 2020-02-17 to 2020-02-21, wait 10 days to compare results, and enable back from 2020-03-02
            // We will allow the post types very late to allow other wpse plugins to register their own post types
            
            if ( date( 'Y-m-d' ) >= '2020-02-17' && date( 'Y-m-d' ) <= '2020-02-21' || date( 'Y-m-d' ) >= '2020-03-02' || (int) get_option( 'vgse_can_edit_cpt_free' ) ) {
                update_option( 'vgse_can_edit_cpt_free', 1 );
                add_filter( 'vg_sheet_editor/allowed_post_types', array( $this, 'allow_all_post_types' ), 99 );
                add_action( 'vg_sheet_editor/editor/before_init', array( $this, 'filter_columns_settings' ), 99 );
            }
