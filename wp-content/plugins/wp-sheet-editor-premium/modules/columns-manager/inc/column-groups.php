		...
		function register_groups_toolbar_items($editor) {
			...
			foreach ($post_types as $post_type) {
				...
				foreach ($existing[$post_type] as $group_key => $group) {
					...
					$delete_button_attribute = ( $active_group_key && $group_key === $active_group_key ) ? 'data-active-item' : 'data-saved-item';
					$editor->args['toolbars']->register_item('cm_g_' . $group_key, array(
						...
						'extra_html_attributes' => 'data-saved-type="columns_manager" ' . $delete_button_attribute . ' data-item-name="' . esc_attr($group_key) . '" ',
							), $post_type);
				}
			}
		}
