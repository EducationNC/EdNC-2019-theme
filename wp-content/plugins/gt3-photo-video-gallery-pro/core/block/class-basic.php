...
abstract class Basic {
	...
	public function initHandler(){
		...
		if(function_exists('register_block_type')) {
			register_block_type('gt3pg-pro/'.$this->name, array(
	...
	protected function ajaxCheckValue(&$value, $type = 'bool'){
		switch($type) {
			case 'bool':
				if(isset($value) && ($value === true || mb_strtolower($value) === 'true' || (int) $value === 1)) {
					$value = true;
				} else {
					$value = false;
				}
	...
	protected function checkImagesNoEmpty($settings){
		if($this->is_editor && !count($settings['ids'])) {
			echo esc_html__('Select Images in Editor', 'gt3pg_pro');
		}
	}
